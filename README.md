...
Download the Lab14_Tester.java file.
...
Testing/Grading
There are test cases to check most of Ir client and server functionality. I will receive the score I see on the submitserver. See the rubric at the bottom of this page.
...
Client
I will implement a client in Client.java that connects to the server with a handshake, and then sends and receives messages. See the unit tests for details of what methods I are required to have (and their functionality).

Server
I will implement a server in Server.java that allows multiple clients to simultaneously connect to the server, perform a handshake, and request that the server factorize an integer.
...
See the unit tests for details of what additional methods I are required to have (and their functionality).

Junit testing
To run Ir unit tests – remember, I need to have the junit jar file in the same directory – compile and run Ir code with the following commands on the terminal (depending on Ir OS):
